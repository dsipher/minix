...
/* the code generator, by design, creates a lot of sequences like

                        I_LIR_op    src2, src1, dst
                        I_LIR_MOVE  dst, src1

   usually register coalescing simply does the Right Thing(tm) and
   dst coalesces away, but in some rare circumstances dst and src1
   look like they interfere somewhere else (usually as a result of
   copy propagation) and thus they can't be merged.

   right before lowering, we rearrange the above as:

                        I_LIR_op    src2, src1, src1
                        I_LIR_MOVE  src1, dst

   which preserves the meaning while throwing the allocator a bone.
   (a similar transformation applies to unary ops, mutatis mutandis.) */
...
void opt_lir_peep(void)
{
    ...
    FOR_ALL_BLOCKS(b)
        FOR_EACH_INSN(b, i, insn) {
            ...
            if (i == (NR_INSNS(b) - 1)) break;      /* no following insn... */

            if (INSN(b, i + 1)->op != I_LIR_MOVE)   /* ...or following insn */
                continue;                           /* isn't an I_LIR_MOVE. */

            move = INSN(b, i + 1);
            from = &move->operand[1];
            to = &move->operand[0];
            if (!OPERAND_REG(from)) continue;       /* O_IMM isn't useful */

            if (I_LIR_UNARY(insn->op)) {
                struct operand *dst = &insn->operand[0];
                struct operand *src = &insn->operand[1];

                /* OP src, dst      ->  OP src, src
                   MOVE dst, src        MOVE src, dst */
                ...
                {
                    dst->reg = src->reg;
                    SWAP(int, from->reg, to->reg);
                }
            } else if (I_LIR_BINARY(insn->op)) {
                struct operand *dst = &insn->operand[0];
                struct operand *src1 = &insn->operand[1];
                struct operand *src2 = &insn->operand[2];

                if (OPERAND_REG(src2) && (src2->reg == dst->reg))
                {
                    commute_insn(insn);             /* try to get dst */
                    src1 = &insn->operand[1];       /* into lhs (src1) */
                    src2 = &insn->operand[2];       /* refresh the srcs */
                }

                /* OP src2, src1, dst   ->  OP src2, src1, src1
                   MOVE dst, src1           MOVE src1, dst */
