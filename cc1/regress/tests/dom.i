...
# 14 "dom.c"
static struct block_vector all;
static struct block_vector tmp1, tmp2;
static struct block_vector stack;
...
static int dom0(struct block *b)
{
    ...
    do { int _n = (0); if (_n <= (((tmp1)).cap)) (((tmp1)).size) = _n; else vector_insert((struct vector *) &((tmp1)), (((tmp1)).size), _n - (((tmp1)).size), sizeof(*(((tmp1)).elements))); } while (0);
    do { int _n = (0); if (_n <= (((tmp2)).cap)) (((tmp2)).size) = _n; else vector_insert((struct vector *) &((tmp2)), (((tmp2)).size), _n - (((tmp2)).size), sizeof(*(((tmp2)).elements))); } while (0);

    for (n = 0; n < (((b)->preds).size); ++n)
        if (n == 0)
            do { int _dummy = &(tmp1) == &((((b)->preds).elements[(n)])->dom); dup_vector((struct vector *) &(tmp1), (struct vector *) &((((b)->preds).elements[(n)])->dom), sizeof(*((tmp1).elements))); } while (0);
        else {
            intersect_blocks(&tmp2, &tmp1, &(((b)->preds).elements[(n)])->dom);
            do { struct block_vector _tmp; _tmp = (tmp1); (tmp1) = (tmp2); (tmp2) = _tmp; } while (0);
        }
    ...
    do { int _dummy = &(b->dom) == &(tmp1); dup_vector((struct vector *) &(b->dom), (struct vector *) &(tmp1), sizeof(*((b->dom).elements))); } while (0);
    return 1;
}
...
static void dom(void)
{
    ...
    for ((b) = all_blocks; (b); (b) = (b)->next)
        if (b == entry_block)
            ...
            do { int _dummy = &(b->dom) == &(all); dup_vector((struct vector *) &(b->dom), (struct vector *) &(all), sizeof(*((b->dom).elements))); } while (0);
...
static void dom_tree(void)
{
    ...
    for ((b) = all_blocks; (b); (b) = (b)->next) {
        for ((i) = 0; ((i) < ((b->dom).size)) && ((a) = (((b->dom)).elements[(i)])); ++(i)) {
            if (a == b) goto next_a;

            for ((j) = 0; ((j) < ((b->dom).size)) && ((c) = (((b->dom)).elements[(j)])); ++(j)) {
                if ((c == a) || (c == b)) continue;
                if (contains_block(&(c)->dom, (a))) goto next_a;
            }
...
static void insert0(struct block *d, struct block *m)
{
    if (!contains_block(&d->loop, m)) {
        add_block(&d->loop, m);
        do { do { int _n = (1); if (((((stack)).size) + _n) < (((stack)).cap)) (((stack)).size) += _n; else vector_insert((struct vector *) &((stack)), (((stack)).size), _n, sizeof(*(((stack)).elements))); } while(0); ((stack).elements[(stack).size - 1]) = (m); } while (0);
    }
}
...
static void loop0(struct block *d, struct block *n)
{
    ...
    do { int _n = (0); if (_n <= (((stack)).cap)) (((stack)).size) = _n; else vector_insert((struct vector *) &((stack)), (((stack)).size), _n - (((stack)).size), sizeof(*(((stack)).elements))); } while (0);
    add_block(&d->loop, d);
    insert0(d, n);

    while (((stack).size)) {
        m = (((stack)).elements[((stack).size) - 1]);
        (((stack).size) -= 1);

        for (i = 0; i < (((m)->preds).size); ++i)
            insert0(d, (((m)->preds).elements[(i)]));
    }
}
...
static void dom_loop(void)
{
    ...
    for ((n) = all_blocks; (n); (n) = (n)->next) {
        for (i = 0; i < (((n)->succs).size); ++i) {
            d = (((n)->succs).elements[(i)]).b;

            if (contains_block(&(n)->dom, (d)))
                loop0(d, n);
        }
    }
    ...
    for ((n) = all_blocks; (n); (n) = (n)->next)
        for ((i) = 0; ((i) < ((n->loop).size)) && ((d) = (((n->loop)).elements[(i)])); ++(i)) {
            d->depth++;
...
void dom_analyze(int flags)
{
    ...
    do { (all).cap = 0; (all).size = 0; (all).elements = 0; (all).arena = (&local_arena); } while (0);
    do { (tmp1).cap = 0; (tmp1).size = 0; (tmp1).elements = 0; (tmp1).arena = (&local_arena); } while (0);
    do { (tmp2).cap = 0; (tmp2).size = 0; (tmp2).elements = 0; (tmp2).arena = (&local_arena); } while (0);
    do { (stack).cap = 0; (stack).size = 0; (stack).elements = 0; (stack).arena = (&local_arena); } while (0);

    for ((b) = all_blocks; (b); (b) = (b)->next) {
        b->depth = 0;
        b->idom = 0;
        do { int _n = (0); if (_n <= (((b->loop)).cap)) (((b->loop)).size) = _n; else vector_insert((struct vector *) &((b->loop)), (((b->loop)).size), _n - (((b->loop)).size), sizeof(*(((b->loop)).elements))); } while (0);
        do { int _n = (0); if (_n <= (((b->dom)).cap)) (((b->dom)).size) = _n; else vector_insert((struct vector *) &((b->dom)), (((b->dom)).size), _n - (((b->dom)).size), sizeof(*(((b->dom)).elements))); } while (0);

        add_block(&all, b);
    }

    dom();
    if (flags & 0x00000001) dom_tree();
    if (flags & 0x00000002) dom_loop();
...
void out_dom(struct block *b)
{
    ...
    for ((n) = 0; ((n) < ((b->dom).size)) && ((tmp) = (((b->dom)).elements[(n)])); ++(n)) {
        out(" %L", tmp->asmlab);
        if (tmp == b->idom) (--(out_f)->_count >= 0 ? (int) (*(out_f)->_ptr++ = (('*'))) : __flushbuf((('*')),(out_f)));
    }

    (--(out_f)->_count >= 0 ? (int) (*(out_f)->_ptr++ = (('\n'))) : __flushbuf((('\n')),(out_f)));

    if (((b->loop).size) || b->depth) {
        out("; LOOP: [depth=%d]", b->depth);

        for ((n) = 0; ((n) < ((b->loop).size)) && ((tmp) = (((b->loop)).elements[(n)])); ++(n))
            out(" %L", tmp->asmlab);
