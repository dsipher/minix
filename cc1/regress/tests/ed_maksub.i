...
char *maksub(char *sub, int subsz)
{
    ...
    for (size = 0; *inptr != delim && *inptr != '\n' && size < subsz; size++) {
        if (*inptr == '&') {
            *cp++ = '&';
            inptr++;
        } else if ((*cp++ = *inptr++) == '\\') {
            if (size >= subsz) return ((void *) 0);

            switch ((*inptr>='a'&&*inptr<='z'?*inptr-32:*inptr))
            {
            case '\n':    *cp++ = '\\';         break;
            case 'S':   *cp++ = 0x20; inptr++;    break;
            case 'N':   *cp++ = '\n'; inptr++;    break;
            case 'T':   *cp++ = 0x09; inptr++;    break;
            case 'B':   *cp++ = 0x08; inptr++;    break;
            case 'R':   *cp++ = 0x0d; inptr++;    break;
