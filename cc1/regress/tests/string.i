...
static struct string *buckets[(1 << 8)];
static unsigned long filters[(1 << 8)][(1 << 3)];
...
static struct slab string_slab = { sizeof(struct string), (100) };
...
struct string *string(char *text, size_t len, int arena)
{
    ...
    if (len > (2147483647 - 1))
        error(1, 0, "string overflow");

    hash = 0xFFFFFFFF;
    for (i = 0; i < len; ++i) hash = (crctab[((hash) ^ ((text[i]) & 0xFF)) & 0xFF] ^ ((hash) >> 8));
    hash ^= 0xFFFFFFFF;
    ...
    if (filters[b][f] & (1L << bit)) {
        for (p = &buckets[b]; s = *p; p = &(s->link))
        {
            ...
            *p = s->link;
            if (arena) do { struct arena *_a = (&string_arena); _a->top = _a->bottom; } while (0);
            break;
        }
    }

    if (s == 0) {
        s = ({ struct slab_obj *_obj; if (_obj = string_slab.free) string_slab.free = _obj->next; else _obj = refill_slab(&string_slab); --string_slab.avail; ((struct string *) (_obj)); });
        s->hash = hash;
        s->len = len;
        s->text = text;
        s->asmlab = 0;
        s->k = 0;
        filters[b][f] |= (1L << bit);
        if (arena) do { struct arena *_a = (&string_arena); _a->bottom = _a->top; } while (0);
    }
...
void out_literal(struct string *s, int n)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (i % 8)
            ...
            if (i) (--(out_f)->_count >= 0 ? (int) (*(out_f)->_ptr++ = (('\n'))) : __flushbuf((('\n')),(out_f)));
            fputs((" .byte "), out_f);
        }
...
struct symbol *literal(struct string *s)
{
    ...
    type = get_tnode(0x0000000000004000L, s->len + 1, &char_type);
