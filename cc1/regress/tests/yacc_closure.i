...
set_EFF(void)
{
    ...
    rowsize = (((nvars)+(32-1))/32);
    EFF = ((unsigned*)allocate((unsigned)((nvars * rowsize)*sizeof(unsigned))));
	...
	for (rule = *sp; rule > 0; rule = *++sp)
	{
	    ...
	    {
		symbol -= start_symbol;
		((row)[(symbol)>>5]|=((unsigned)1<<((symbol)&31)));
	    }
	}
	row += rowsize;
    }
...
set_first_derives(void)
{
    ...
    rulesetsize = (((nrules)+(32-1))/32);
    varsetsize = (((nvars)+(32-1))/32);
    first_derives = ((unsigned*)allocate((unsigned)((nvars * rulesetsize)*sizeof(unsigned)))) - ntokens * rulesetsize;
	...
	for (j = start_symbol; j < nsyms; k++, j++)
	{
		...
		while ((rule = *rp++) >= 0)
		{
		    ((rrow)[(rule)>>5]|=((unsigned)1<<((rule)&31)));
		}
	    }
	}
...
finalize_closure(void)
{
    (free((char*)(itemset)));
    (free((char*)(ruleset)));
    (free((char*)(first_derives + ntokens * (((nrules)+(32-1))/32))));
}
