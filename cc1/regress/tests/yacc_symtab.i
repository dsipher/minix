...
hash(char *name)
{
    ...
    if (!(name && *name)) __assert("name && *name", "symtab.c", 36);;
    s = name;
    k = *s;
    while (c = *++s)
	k = (31*k + c) & (1024 - 1);
...
make_bucket(char *name)
{
    ...
    if (!(name)) __assert("name", "symtab.c", 51);;
    bp = (bucket *) (malloc((unsigned)(sizeof(bucket))));
    if (bp == 0) no_space();
    bp->link = 0;
    bp->next = 0;
    bp->name = (malloc((unsigned)(strlen(name) + 1)));
...
create_symbol_table(void)
{
    ...
    symbol_table = (bucket **) (malloc((unsigned)(1024*sizeof(bucket *))));
