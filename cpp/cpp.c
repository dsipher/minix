...
/* synchronize the output file's idea of path/line_no with ours. usually
   this simply emits newlines, but when large (more than RESYNC_WINDOW)
   line changes or the path changes, output a #line directive. the flag
   need_resync is a dirty hack; input.c and directive.c set this when the
...
static void resync(void)
{
    ...
    if (need_resync || (line_no > INPUT_STACK->line_no)
      || (line_no < (INPUT_STACK->line_no - RESYNC_WINDOW))) {
        line_no = INPUT_STACK->line_no;
        fprintf(out_fp, "\n# %d \"%s\"\n", line_no,
...
static void loop(void)
{
    ...
    fprintf(out_fp, "# 1 \"%s\"\n", VSTRING_BUF(INPUT_STACK->path));
    need_resync = 0; /* this kludge ensures at least one #line is output */
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "xI:D:")) != EOF) {
        switch (opt)
        {
    ...
    if ((argc - optind) != 2) {
        fprintf(stderr, "cpp: incorrect number of arguments\n");
        goto usage;
    }
    ...
    input_open(argv[optind], INPUT_SEARCH_NOWHERE);
